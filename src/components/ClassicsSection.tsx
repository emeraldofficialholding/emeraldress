...
const ClassicsSection = () => {
  const {
    data: products,
    isLoading
  } = useProducts("classics");
  return <section className="py-20 bg-accent relative shadow-2xl">
      <FloatingButterflies />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
          <div className="lg:col-span-3 lg:sticky lg:top-32">
            ...
            once: true
              ...
              <Link to="/collezioni" className="inline-block border border-foreground px-6 py-2.5 text-xs tracking-[0.15em] uppercase font-sans hover:bg-foreground hover:text-background transition-all duration-500">
                Scopri di più
              </Link>
            </motion.div>
          </div>
          ...
          <div className="lg:col-span-9">
            {isLoading ? <ButterflyLoader /> : <Carousel opts={{
            align: "start",
            loop: true
          }} className="w-full">
                <CarouselContent className="-ml-4">
                  {products?.map(product => <CarouselItem key={product.id} className="pl-4 basis-full sm:basis-1/2 lg:basis-1/3">
                      <Link to={`/product/${product.id}`} className="group block">
                        <div className="aspect-[3/4] overflow-hidden bg-muted mb-3 shadow-sm transition-shadow duration-300 group-hover:shadow-lg">
                          <ImageFallback src={product.images[0]} hoverSrc={product.images[1]} alt={product.name} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
                        </div>
