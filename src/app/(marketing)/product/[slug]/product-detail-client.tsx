            ...
            <thead>
              <tr className="border-b border-border">
                {["Taglia", "Busto", "Vita", "Fianchi"].map((h) => (
                  <th key={h} className="py-2 text-left text-muted-foreground tracking-widest uppercase font-normal">
                    {h}
                  </th>
                ))}
              </tr>
    ...
    <div className="hidden lg:flex gap-3 h-full">
      ...
      <div className="relative flex-1 overflow-hidden group">
        <AnimatePresence mode="wait">
          ...
          >
            <ImageFallback src={images[activeImage]} alt={productName} className="w-full h-full object-cover" />
          </motion.div>
        </AnimatePresence>
          ...
          {images.map((_, i) => (
            <button
              ...
              className={cn(
                ...
                currentIndex === i ? "w-5 h-1.5 bg-foreground" : "w-1.5 h-1.5 bg-muted-foreground/40",
              )}
            />
          ))}
        </div>
        ...
        <div className="max-w-[1400px] mx-auto px-4 md:px-8">
          ...
          <div className="lg:grid lg:grid-cols-[1fr_420px] xl:grid-cols-[1fr_480px] lg:gap-16 xl:gap-24">
            ...
            >
              ...
              <p className="font-sans text-xl tracking-wide mb-6">€{Number(product.price).toFixed(2)}</p>
              ...
              <div className="mb-6">
                <div className="flex items-center justify-between mb-3">
                  <p className="font-sans text-[10px] tracking-[0.25em] uppercase text-muted-foreground">
                    Taglia
                    {selectedSize && <span className="ml-2 text-foreground font-medium">— {selectedSize}</span>}
                  </p>
                  <button
                  ...
                  <AccordionContent className="text-xs font-sans text-muted-foreground leading-relaxed pb-5 space-y-2">
                    <p>
                      {product.shipping_info ||
                        "Spedizione standard 3–5 giorni lavorativi. Spedizione espressa disponibile al checkout."}
                    </p>
