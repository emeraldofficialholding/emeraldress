...
const N8N_SCANNER_URL =
  process.env.NEXT_PUBLIC_N8N_SCANNER_URL ?? "https://n8n.kreareweb.com/webhook/scanner-requests";

export function EmeraldScannerClient() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const {
        data: { session: scanSession },
      } = await supabase.auth.getSession();
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const { data, error } = await (supabase.from("scanner_requests") as any)
        .insert([
          {
            image_url: imageUrl,
      ...
      const webhookRes = await fetch(N8N_SCANNER_URL, {
        method: "POST",
      ...
      const dbScore = typeof row?.sustainability_score === "number" ? row.sustainability_score : null;
      const rawDiag = row?.diagnosis_result;
      const dbDiagnosis =
        rawDiag == null ? null : typeof rawDiag === "string" ? rawDiag : JSON.stringify(rawDiag);
      ...
      if (recordId) {
        try {
          ...
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const row = dbRow as any;
          if (row?.sustainability_score && row.sustainability_score > 0) {
            setResultScore(row.sustainability_score);
            const r = row.diagnosis_result;
              ...
              <AnimatePresence mode="wait">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 flex-1">
                        ...
                        >
                          {previewUrl ? (
                            ...
                            <>
                              ...
                              <h2 className="font-serif text-base text-neutral-900 mb-1">Carica Foto</h2>
                        ...
                        <div className="flex flex-col justify-center space-y-4 font-sans">
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              <Label htmlFor="brand" className="text-[10px] uppercase text-neutral-400 mb-1 block tracking-wider font-sans">
                                Brand
                              </Label>
                            ...
                            <div>
                              <Label htmlFor="garment" className="text-[10px] uppercase text-neutral-400 mb-1 block tracking-wider font-sans">
                                Tipo Capo
                              </Label>
                          ...
                          <div>
                            <Label htmlFor="material" className="text-[10px] uppercase text-neutral-400 mb-1 block tracking-wider font-sans">
                              Materiale
                            </Label>
                          ...
                          <div>
                            <Label htmlFor="quality" className="text-[10px] uppercase text-neutral-400 mb-1 block tracking-wider font-sans">
                              Qualità Percepita
                            </Label>
                ...
                {(phase === "uploading" || phase === "waiting") && (
                  ...
                  >
                    ...
                    <div className="text-center space-y-3 max-w-sm">
                      <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="font-serif text-xl md:text-2xl text-neutral-800 italic leading-snug">
                        {phase === "uploading"
                          ? "Preparazione dell'analisi..."
                          : "La nostra consulente tessile sta analizzando la fibra..."}
                      </motion.p>
                      {phase === "waiting" && (
                        <motion.p initial={{ opacity: 0 }} animate={{ opacity: [0.3, 0.7, 0.3] }} transition={{ duration: 3, repeat: Infinity }} className="text-[10px] uppercase tracking-[0.25em] text-neutral-400 font-sans">
                          Analisi in corso
                        </motion.p>
                      )}
                    </div>
                ...
                {phase === "result" &&
                  (() => {
                    ...
                    if (!isValid) {
                      return (
                        <motion.div key="result-fallback" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="w-full max-w-md h-full flex flex-col items-center justify-center text-center px-6">
                          <div className="bg-white rounded-[2rem] p-10 md:p-14 border border-neutral-100 shadow-[0_8px_60px_-12px_rgba(0,0,0,0.06)]">
                            ...
                            <button onClick={resetScanner} className="bg-neutral-900 text-white rounded-md px-8 py-3 font-serif uppercase text-sm tracking-wider transition-all hover:bg-stone-800">
                    ...
                    return (
                      <motion.div key="result-state" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: "easeOut" }} className="w-full max-w-lg flex flex-col items-center justify-start py-4 pb-20">
                        <div className="bg-white w-full rounded-[2rem] p-8 md:p-10 text-center shadow-[0_8px_60px_-12px_rgba(0,0,0,0.08)] border border-neutral-100 relative overflow-hidden break-words">
                          ...
                          <div className="relative w-36 h-36 mx-auto mb-4 mt-2">
                            ...
                            <div className="absolute inset-0 flex flex-col items-center justify-center">
                              <motion.span initial={{ opacity: 0, scale: 0.5 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5, delay: 0.5 }} className={cn("font-serif text-5xl font-light leading-none", sc.text)}>
                                {score}
                              </motion.span>
                            </div>
                          </div>

                          <p className="text-[9px] uppercase tracking-[0.3em] text-neutral-400 font-sans mb-1">Sustainability Score</p>
                          <p className={cn("text-xs uppercase tracking-[0.3em] font-sans mb-6", sc.labelColor)}>{sc.label}</p>

                          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8 }} className="mb-6 px-4 md:px-10">
                            <div className="w-8 h-px bg-stone-200 mx-auto mb-5" />
                            <p className="font-serif text-sm md:text-base text-stone-800 text-center max-w-xs mx-auto break-words whitespace-normal" style={{ lineHeight: 1.8, letterSpacing: "0.01em", overflowWrap: "anywhere" }}>
                              {diagnosisText}
                            </p>
                          </motion.div>

                          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.1 }} className="bg-emerald-50/30 rounded-2xl p-6 border border-emerald-100/50 mb-6">
                            ...
                            <div className="space-y-4">
                              <div className="flex items-center gap-3">
                                ...
                                <div className="flex-1 h-2 bg-stone-100 rounded-full overflow-hidden">
                                  <motion.div className="h-full bg-stone-400 rounded-full" initial={{ width: 0 }} animate={{ width: `${score}%` }} transition={{ duration: 1.2, delay: 1.3, ease: "easeOut" }} />
                                </div>
                              ...
                              <div className="flex items-center gap-3">
                                ...
                                <div className="flex-1 h-2 bg-emerald-100 rounded-full overflow-hidden">
                                  <motion.div className="h-full bg-emerald-600 rounded-full" initial={{ width: 0 }} animate={{ width: "100%" }} transition={{ duration: 1.2, delay: 1.5, ease: "easeOut" }} />
                                </div>
                                <span className="text-xs font-serif text-emerald-700 w-12 text-left">100/100</span>
                              </div>
                            </div>

                            <p className="text-[10px] text-stone-500 font-sans mt-4 leading-relaxed tracking-wide">Produzione Etica · Tessuti Rigenerati ECONYL® · Zero Sprechi</p>
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-12 max-w-6xl mx-auto">
            <div className="w-full md:w-5/12 space-y-8 text-center md:text-left">
              ...
              <p className="text-lg text-neutral-600 font-sans leading-relaxed">
                Il nostro algoritmo si basa sui 4 pilastri della sostenibilità Emeraldress. Un ciclo virtuoso che trasforma lo scarto in bellezza e garantisce un futuro rigenerabile.
              </p>
              <div className="flex justify-center md:justify-start pt-2">
                <Link href="/sostenibilita">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-y-10 gap-x-4 relative">
                  {pillars.map(({ Icon, title, desc }, index) => (
                    <div key={index} className="flex flex-col items-center text-center relative group cursor-default">
                      <motion.div whileHover={{ scale: 1.1, rotate: 10, backgroundColor: "#fff" }} className="w-16 h-16 rounded-full bg-white border-2 border-emerald-100 flex items-center justify-center mb-4 relative z-10 shadow-sm transition-colors duration-300">
                        <Icon className="w-7 h-7 text-emerald-600" />
                      </motion.div>
                      <h3 className="font-serif text-lg text-emerald-950 mb-1">{title}</h3>
                      <span className="text-[10px] tracking-widest uppercase text-neutral-400 font-sans font-bold">{desc}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} viewport={{ once: true }}>
            ...
            <div className="flex justify-center">
              <Link href="/collezioni">
                <HoverBorderGradient containerClassName="rounded-full" className="bg-[#e4ffec] text-emerald-950 flex items-center gap-3 px-10 py-5 font-bold tracking-widest uppercase text-sm">
